    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
    	<thead>
    		<tr>
    			...
    			<th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'yiw' ) ?></span></th>
    			<th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'yiw' ) ?></span></th>
    			<th><span class="nobr"><?php _e( 'Stock Status', 'yiw' ) ?></span></th>
            ...
            if( count( $wishlist ) > 0 ) :
                foreach( $wishlist as $values ) :   
                    ...
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'yiw' ) ?>">&times;</a></td>
                        ...
                        <td class="product-stock-status">
                            ...
                            if( $stock_status == 'out-of-stock' ) {
                                $stock_status = "Out";
                                echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'yiw' ) . '</span>';   
                            } else {
                                $stock_status = "In";
                                echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'yiw' ) . '</span>';
                            }
                            ?>
                        </td>
                ...
                <tr>
                    <td colspan="6" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'yiw' ) ?></td>
                </tr>       
            <?php
            endif;
