    ...
    class YITH_WCWL {
        ...
        public function is_product_in_wishlist( $product_id ) {
    		...
    		if( is_user_logged_in() ) {		
    			...
    			$exists = isset( $results[0] ) && $results[0]->cnt > 0 ? true : false;
    		} else {
                if( yith_usecookies() ) {
        ...
        public function add() {
            ...
            if ( isset( $this->details['add_to_wishlist'] ) && is_numeric( $this->details['add_to_wishlist'] ) ) {
                ...
                if( $return ) {
                    ...
                    $this->errors[] = __( 'Error occurred while adding product to wishlist.', 'yiw' );
                    return "error";
                }
            } 
        }
        ...
        public function remove( $id ) {
            ...
            if( is_user_logged_in() ) {
                ...
                if( $wpdb->query( $sql ) ) {
                    ...
                    $this->errors[] = __( 'Error occurred while removing product from wishlist', 'yiw' );
                    return false;
                }
            } elseif( yith_usecookies() ) {
        ...
        public function add_to_wishlist_ajax() {    
            ...
            if( $return == 'true'  )
                { echo $return . '##' . __( 'Product added!', 'yiw' ); }
            elseif( $return == 'exists' )
                { echo $return . '##' . __( 'Product already in the wishlist.', 'yiw' ); }
        ...
        public function remove_from_wishlist_ajax() {   
            ...
            if( $this->remove( $_GET['wishlist_item_id'] ) )
                { _e( 'Product successfully removed.', 'yiw' ); }
            else {
                echo '#' . $count . '#';
                _e( 'Error. Unable to remove the product from the wishlist.', 'yiw' );
            }
            
            if( !$count )
                { _e( 'No products were added to the wishlist', 'yiw' ); }
